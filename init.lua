...
hacktool.formspec = function(user_name, player, inventory_name)
	local formspec = ("size[%f,5]"):format(formspec_width)

	for i, inv_def in ipairs(inventories) do
		formspec = formspec .. ("button[%f,0;2,1;inventory:%s;%s]"):format((i - 1) * 2, inv_def.name, inv_def.label)
	end

	formspec = formspec .. inventory_formspec(inventory_name, player:get_inventory(), (formspec_width - 8) / 2, 1)

	return formspec
end

minetest.register_privilege("invhack", {
	description = "Lets you hack player's inventories",
